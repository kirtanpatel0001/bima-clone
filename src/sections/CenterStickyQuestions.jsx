...
export default function CenterStickyQuestions({
  ...
  // Improved IntersectionObserver: picks section with highest intersection ratio
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        // Filter intersecting sections
        const visible = entries
          .filter((e) => e.isIntersecting)
          .sort((a, b) => b.intersectionRatio - a.intersectionRatio)[0]; // pick most visible one
  ...
  return (
    <section
      ref={containerRef}
      className="relative w-full bg-[#000319] text-white overflow-hidden"
    >
      ...
      <div
        ref={scrollAreaRef}
        className="relative h-[210vh] mx-auto overflow-y-auto no-scroll"
      >
        {/* Sticky center title */}
          ...
          <h1 className="text-6xl md:text-8xl lg:text-[120px] font-extrabold text-center leading-tight">
            {centerTitle.split(" ").map((word, i) => (
              <span
                key={i}
                className={
                  word.toLowerCase().includes("you") ? "text-purple-400" : ""
                }
              >
                {word + " "}
              </span>
          ...
          {items.map((text, i) => {
            ...
            return (
              <div
                key={i}
                data-index={i}
                className="scroll-section h-[120vh] flex items-center justify-center relative"
              >
                <AnimatePresence mode="wait">
                  {activeIndex === i && (
                    <motion.div
                      ...
                      animate="animate"
                      exit="exit"
                      className={`absolute max-w-xs md:max-w-md text-lg md:text-2xl font-medium ${
                        isLeft ? "left-12 text-left" : "right-12 text-right"
                      }`}
                    >
                      <div className="relative p-4">
                        <span
                          className={`absolute w-5 h-5 border border-white/20 ${
                            isLeft ? "-left-4 -top-4" : "-right-4 -top-4"
                          }`}
                        />
                        <span
                          className={`absolute w-5 h-5 border border-white/20 ${
                            isLeft ? "-left-4 -bottom-4" : "-right-4 -bottom-4"
                          }`}
                        />
                        <p>{text}</p>
                      </div>
